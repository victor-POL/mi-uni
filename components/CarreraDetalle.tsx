...
export function CarreraDetalle({ carrera, usuarioId }: CarreraDetalleProps) {
  ...
  const fetchHistorial = async () => {
    ...
    try {
      const response = await fetch(
        `/api/user/carreras/materias?userId=${usuarioId}&planEstudioId=${carrera.planEstudioId}`
      )

      if (!response.ok) throw new Error('Error cargando historial')

      const result: ApiResponse<HistorialData> = await response.json()

      if (!result.success) {
        throw new Error(result.error || 'Error desconocido al cargar historial')
      }
  ...
  const formatearNota = (nota?: number | null) => {
    ...
    return !Number.isNaN(num) && num !== 0 ? num.toFixed(2) : 'N/A'
  }
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>Plan {carrera.planEstudioAnio} - Detalle completo de tu progreso académico</CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4">
                    ...
                    <div className="text-center">
                      <p className="text-2xl font-bold text-blue-600">
                        {carrera.materiasTotal - carrera.materiasAprobadas}
                      </p>
                    ...
                    <Card>
                      <CardContent className="p-4">
                        ...
                        <p className="text-2xl font-bold text-purple-600">
                          {materiasEnCurso.estadisticas.materiasAnual}
                        </p>
                      </CardContent>
                    </Card>
                    <Card>
                      <CardContent className="p-4">
                        ...
                        <p className="text-2xl font-bold text-green-600">
                          {materiasEnCurso.estadisticas.materiasPrimero}
                        </p>
                      </CardContent>
                    </Card>
                    <Card>
                      <CardContent className="p-4">
                        ...
                        <p className="text-2xl font-bold text-orange-600">
                          {materiasEnCurso.estadisticas.materiasSegundo}
                        </p>
                      </CardContent>
                    </Card>
                    <Card>
                          ...
                          <TableBody>
                            {materiasEnCurso.materias.map((materia) => (
                              <TableRow key={`${materia.id}-${materia.anioCursada}-${materia.cuatrimestreCursada}`}>
                                ...
                                <TableCell>
                                  {materia.anioCursada} - {formatearCuatrimestre(materia.cuatrimestreCursada)}
                                </TableCell>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    ...
                    <Card>
                      <CardContent className="p-4">
                        ...
                        <p className="text-2xl font-bold text-blue-600">
                          {historial.estadisticas.materiasEnCurso || 0}
                        </p>
                      </CardContent>
                    </Card>
                    <Card>
                      <CardContent className="p-4">
                        ...
                        <p className="text-2xl font-bold text-red-600">
                          {historial.estadisticas.materiasPendientes || 0}
                        </p>
                      </CardContent>
                    </Card>
                    <Card>
                          ...
                          <TableBody>
                            {historial.historial.map((materia) => (
                              <TableRow key={materia.id} className="hover:bg-gray-50">
                                ...
                                <TableCell>{materia.anioCursada || '-'}</TableCell>
                                <TableCell>
                                  {materia.cuatrimestreCursada
                                    ? formatearCuatrimestre(materia.cuatrimestreCursada)
                                    : '-'}
                                </TableCell>
                                <TableCell>{materia.horasSemanales}hs</TableCell>
                                <TableCell>

    ...
    const result = await query(
      `SELECT 
       ...
       WHERE pm.plan_estudio_id = $1
         AND NOT EXISTS (
           SELECT 1 FROM prod.usuario_materia_cursada umc 
           WHERE umc.usuario_id = $2 
             AND umc.plan_estudio_id = $1 
             AND umc.materia_id = m.id
         )
  ...
  try {
    ...
    await query(
      `INSERT INTO prod.usuario_materia_cursada 
       (usuario_id, plan_estudio_id, materia_id, anio_cursada, cuatrimestre_cursada, fecha_actualizacion)
    ...
    const result = await query(
      `DELETE FROM prod.usuario_materia_cursada 
       WHERE usuario_id = $1 
         ...
         AND materia_id = $3 
         AND anio_cursada = $4 
         AND cuatrimestre_cursada = $5`,
      [usuarioId, planEstudioId, materiaId, anioCursada, cuatrimestreCursada]
    )
    ...
    const result = await query(
      `SELECT 
         ...
         AVG(CASE 
           ...
           THEN (umc.nota_primer_parcial + umc.nota_segundo_parcial) / 2
